...
**Primary recommendation:** Scaffold with `pnpm create @tanstack/start@latest --tailwind --add-ons shadcn` (simpler and more reliable than the preset URL), then restructure into the monorepo layout with `packages/web`, `packages/db`, `packages/core`, and `packages/worker`. Use `temporal server start-dev` via Docker for the lightest-weight Temporal development setup.
...
|------------|-----------|----------|
| `temporal server start-dev` (Docker) | Full docker-compose (PostgreSQL + Elasticsearch) | Full compose adds Elasticsearch visibility search but is heavyweight for dev. Use lightweight CLI server for Phase 1. |
...
| `temporalio/docker-compose` repo | `temporalio/samples-server` repo | Jan 2026 (archived) | Docker compose files moved. Use `temporalio/auto-setup` image for simplest dev server. |
...
**Deprecated/outdated:**
- `temporalio/docker-compose` GitHub repo: archived Jan 2026. Docker compose examples moved to `temporalio/samples-server`.
...
- [Temporal Docker Compose (archived)](https://github.com/temporalio/docker-compose) - Port mappings, available configurations, auto-setup image
